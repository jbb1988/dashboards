-- Fix: Cannot cast jsonb to text[] directly
-- Problem: Line 57 in 042_fix_sync_operation_transaction.sql uses:
--   COALESCE((c->'contract_type')::jsonb::text[], ARRAY[]::text[])
-- But PostgreSQL cannot cast jsonb directly to text[]
-- Solution: Use jsonb_array_elements_text() to properly extract array elements
...
BEGIN
  ...
  SELECT
    ...
    -- Properly convert JSONB array to TEXT[] using jsonb_array_elements_text
    CASE
...
'Atomically upserts contracts from Salesforce sync. Sets sync operation flag within same transaction to prevent false "pending sync" status. Preserves manual status overrides. Fixed JSONB to text[] conversion.';
