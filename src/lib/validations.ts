...
export const taskCreateSchema = z.object({
  ...
  // Contract fields (mutually exclusive with bundle)
  ...
  bundleName: z.string().max(255).optional(),
  ...
  (data) => {
    // Must have either contract OR bundle, not both, not neither
    const hasContract = !!(data.contractId || data.contractSalesforceId);
    const hasBundle = !!data.bundleId;
    return (hasContract && !hasBundle) || (!hasContract && hasBundle);
  },
  {
    message: 'Must specify either a contract or a bundle, not both',
  }
);
...
export const taskUpdateSchema = z.object({
  ...
  bundleName: z.string().max(255).optional().nullable(),
...
export const documentCreateSchema = z.object({
  ...
  (data) => {
    // Must have either contract OR bundle, not both, not neither
    const hasContract = !!(data.contractId || data.salesforceId);
    const hasBundle = !!data.bundleId;
    return (hasContract && !hasBundle) || (!hasContract && hasBundle);
  },
  {
