...
function mapTaskToCalendarEvent(task: AsanaTask): CalendarEvent {
  return {
    gid: task.gid,
    name: task.name,
    ...
    status: getTaskStatusForCalendar(task),
    tags: task.tags,
    section: task.section,
  };
}

// Helper to get task status for calendar (must be defined before mapTaskToCalendarEvent uses it)
function getTaskStatusForCalendar(task: AsanaTask): 'confirmed' | 'placeholder' | null {
  ...
  const scheduleStatus = task.customFields?.find(f => f.name.toLowerCase() === 'schedule status')?.value as string | null;
  if (scheduleStatus?.toLowerCase()?.includes('confirmed')) return 'confirmed';
  if (scheduleStatus?.toLowerCase()?.includes('placeholder')) return 'placeholder';
  return null;
}
...
function TimelineTab({ data, loading, onTaskComplete }: { data: ProjectData | null; loading: boolean; onTaskComplete?: (taskId: string, completed: boolean) => Promise<void> }) {
  ...
  // Handle event move from calendar drag-and-drop
  const handleEventMove = useCallback(async (eventId: string, newDate: string) => {
    try {
      ...
      // Note: Data will refresh on next fetch or you could trigger a refetch here
    } catch (error) {
  ...
  return (
    <div>
      ...
      {/* Continuous Scroll Calendar View */}
      {viewMode === 'calendar' && (
        <ContinuousCalendar
