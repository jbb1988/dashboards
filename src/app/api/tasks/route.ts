...
export async function POST(request: NextRequest) {
  try {
    ...
    // If bundleId is provided, fetch bundle_name from database
    let bundleName = validated.bundleName;
    if (validated.bundleId && !bundleName) {
      try {
        const bundleResponse = await fetch(`${request.nextUrl.origin}/api/bundles?bundleId=${validated.bundleId}`);
        ...
        return NextResponse.json({ error: 'Failed to fetch bundle information' }, { status: 500 });
      }
    }
    ...
    const task: Omit<Task, 'id' | 'created_at' | 'updated_at'> = {
      ...
      // Bundle fields (NEW)
      bundle_id: validated.bundleId || undefined,
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (updates.contractName !== undefined) taskUpdates.contract_name = updates.contractName || undefined;
    ...
    // Handle bundle updates (NEW)
    if (updates.bundleId !== undefined) {
      taskUpdates.bundle_id = updates.bundleId || undefined;

      // Fetch bundle name if bundleId is set
      if (updates.bundleId && !updates.bundleName) {
        try {
          const bundleResponse = await fetch(`${request.nextUrl.origin}/api/bundles?bundleId=${updates.bundleId}`);
          if (bundleResponse.ok) {
            const bundle = await bundleResponse.json();
            taskUpdates.bundle_name = bundle.name;
          }
        } catch (err) {
          console.warn('Failed to fetch bundle name:', err);
        }
      }

      // Clear contract fields if bundle is set
      if (updates.bundleId) {
        taskUpdates.contract_id = undefined;
        taskUpdates.contract_salesforce_id = undefined;
        taskUpdates.contract_name = undefined;
        taskUpdates.contract_stage = undefined;
      }
    }
    if (updates.bundleName !== undefined) {
      taskUpdates.bundle_name = updates.bundleName || undefined;
    }
